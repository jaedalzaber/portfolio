...
import { Picarry } from '../../../models/Database';
import styles from './AppDesign.module.scss';
...
const AppDesign = React.forwardRef((props, ref) => {
					...
					<div className={styles['grid']} ref={panelsContainer}>
						<div className={styles['motion--header']}>
							<div className={styles.title}>
								<span className={styles.title__bold}>{data.title}</span><br />
						...
						<div className={styles['cover']} ref={(e) => createPanelsRefs(e, 0)}>
							<img className={styles['cover--img']} src={data.cover} alt='' />
						</div>

						<div ref={(e) => createPanelsRefs(e, 1)}>
							<div className={`${styles['viewer']} `}>
								<div className={styles['pi-title']} >
									<p className={styles['pi-title--light']} >Online</p>
									<p className={styles['pi-title--bold']} >Wholesaling</p>
									<p className={styles['pi-title--bold']} >Platform</p>
								</div>
								...
								<p className={styles['subtlt']} >
									Admin Dashboard
								</p>
								<div className={styles['intro']} >
									<p >
										PiCarry is a wholesale platform where retail store owners can easily order product from different wholesellers through our app.
									</p>
									<p>
										The project is consist of three application, one for retailers to place order, one for wholesellers to manage products and order. Finally a master dashboard for Picarry admin to maintain control over every data and transations.</p>
								</div>
							</div>

							<div className={`${styles['section']} ${styles['section-1']} `} >
								<div className={`${styles['section--left']} `}>
									<div className={styles['section--subtitle']} >Mobile App for </div>
									<div className={styles['section--title']} >Retailer</div>
									<div className={styles['section--copy']} >
										In the retailer app, a store owner can search for his needed products, get the latest price and order place order from any wholeseller.
										<br /><br />
										It consists of five pages, Home to get a summary of new, recommended, most bought products etc.
										<br /><br />
										A category page and a product page for product listing.
										<br /><br />
										Finally cart page to see all ordered product and place order.
									</div>
								</div>
								<div className={`${styles['section--right']} `}>
									<div className={`${styles['section--video']} `}>
										<video width="350" height="690" loop autoPlay muted>
											<source src={data.vidRetail} type="video/mp4" />
										</video>
										<div className={styles['section--video-border']} ></div>
									</div>
								</div>
							</div>
							...
							<div className={`${styles['section']} ${styles['section-1']} `} >
								<div className={`${styles['section--right']} `}>
									<div className={`${styles['section--video']} `}>
										<video width="350" height="690" loop autoPlay muted>
											<source src={data.vidSeller} type="video/mp4" />
										</video>
										<div className={styles['section--video-border']} ></div>
									</div>
								</div>
								<div className={`${styles['section--left']} `}>
									<div className={styles['section--subtitle']} >Mobile App for </div>
									<div className={styles['section--title']} >Wholesellers</div>
									<div className={styles['section--copy']} >
										In Seller mobile app, a wholeseller manages his product and orders.
										<br /><br />
										It consists of four pages. Home page shows all the statistics and summary.
										<br /><br />
										Categories and products can easily be created and updated in Category and Product page consequtively.
										<br /><br />
										Order page shows all the new ordes placed by retailers. This orders can be confirmed or cancelled by wholesellers as he need.
									</div>
								</div>
							</div>
						</div>

						<div className={`${styles['feature']} ${styles['feature__first']} `} ref={(e) => createPanelsRefs(e, 2)}>
							<div className={`${styles['feature--title']} ${styles['feature--title__left']} `}>
								<div className={styles['feature--title__light']} >{data.features[0].subtitle}</div>
								<div className={styles['feature--title__bold']} >{data.features[0].title}</div>
							</div>
							<img className={styles['feature--img']} src={data.features[0].img} alt='' />
						</div>
							...
							data.features.map((item, i) => {
								...
								return (
									<div className={`${styles['feature']}`} >
										<div className={`${styles['feature--title']} ${styles[item.pos === 1 ? 'feature--title__right' : 'feature--title__left']} `}>
											{data.features[i].title}
										</div>
										<img className={styles['feature--img']} src={data.features[i].img} alt='' />
									</div>
								)
							})
						}
			...
			<div className={styles['nav']} >
				<div className={styles['nav-wrap']} >
					...
					<div className={styles['arrow-left']} >
						<Link to='/' style={{ textDecoration: 'none' }} state={{ scrollTo: 'ux' }}><GoBackBtn /></Link>
					</div>
				</div>
			</div>
		</LocomotiveScrollProvider>
